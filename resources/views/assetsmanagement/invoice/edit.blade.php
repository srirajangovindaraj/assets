...
@section('content')
 ...
 @if (session()->has('errors'))
            ...
            <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 px-8 py-6">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <h2 class="text-2xl font-bold text-white">Edit Invoice #{{ $invoice->invoice_number }}</h2>
                    </div>
                    <div class="bg-indigo-500/20 px-4 py-2 rounded-lg">
                        <span class="text-sm font-medium text-indigo-100">Edit Mode</span>
                    </div>
                </div>
            </div>

            <form method="POST" action="{{ route('asset-management.invoice.update', $invoice->id) }}" enctype="multipart/form-data"
                class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="mb-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <select id="vendorSearch" name="vendor_id" required
                                    class="w-full px-4 py-3 pl-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 appearance-none bg-white">
                                    <option value="">Select Vendor</option>
                                    @foreach ($vendors as $vendor)
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="invoice_number" required
                                ...
                                placeholder="INV-001" value="{{ old('invoice_number', $invoice->invoice_number) }}">
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                        <div class="space-y-2">
                            ...
                            <select name="category" id="categorySelect"
                                ...
                                <option value="electronics" {{ $invoice->category == 'electronics' ? 'selected' : '' }}>
                                    Electronics</option>
                                <option value="books" {{ $invoice->category == 'books' ? 'selected' : '' }}>Books
                                </option>
                                <option value="furniture" {{ $invoice->category == 'furniture' ? 'selected' : '' }}>
                                    Furniture</option>
                            </select>
                        </div>
                        <div id="partialAmountDiv"
                            class="space-y-2 {{ $invoice->status != 'partial_paid' ? 'hidden' : '' }}">
                            <label class="block text-sm font-medium text-gray-700">
                                <span class="flex items-center">
                                    <svg class="h-4 w-4 text-gray-500 mr-1" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                            ...
                            <input type="number" name="partial_amount" id="partialAmount"
                                ...
                                value="{{ old('partial_amount', $invoice->partial_amount) }}">
                        </div>

                        <div id="duedateDiv"
                            class="space-y-2 {{ in_array($invoice->status, ['unpaid', 'partial_paid']) ? '' : 'hidden' }}">
                            <label class="block text-sm font-medium text-gray-700">
                                <span class="flex items-center">
                                    <svg class="h-4 w-4 text-gray-500 mr-1" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                            ...
                            <input type="date" name="due_date" id="duedate"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200"
                                placeholder="0.00" step="0.01"
                                value="{{ old('invoice_date', optional($invoice->due_date)->format('Y-m-d')) }}">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200">
                        <table class="w-full" id="itemsTable">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <!-- Existing items will be populated here by JavaScript -->
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="mb-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                        <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                            ...
                            <div class="text-2xl font-bold text-gray-900" id="subTotalDisplay">
                                {{ number_format($invoice->sub_total, 2) }}</div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="discountType" name="discount_type"
                                ...
                                <option value="percentage"
                                    {{ $invoice->discount_type == 'percentage' ? 'selected' : '' }}>Percentage %</option>
                                <option value="amount" {{ $invoice->discount_type == 'amount' ? 'selected' : '' }}>Amount
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input id="discountValue"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200"
                                placeholder="0.00"
                                value="{{ old('discount_value', $invoice->discount_percentage ?: $invoice->discount_amount) }}">
                        </div>

                        <div class="bg-blue-50 p-6 rounded-lg border border-blue-200">
                            <label class="block text-sm font-medium text-blue-600 mb-2">Tax Amount</label>
                            <div class="text-2xl font-bold text-blue-900" id="taxAmountDisplay">
                        ...
                        <div class="bg-indigo-50 p-6 rounded-lg border border-indigo-200">
                            ...
                            <div class="text-2xl font-bold text-indigo-900" id="grandTotalDisplay">
                ...
                <div class="flex items-center justify-end pt-8 border-t border-gray-200">
                            ...
                            @if ($invoice->invoice_file_path)
                                <div class="flex items-center">
                                    ...
                                    <a href="{{ asset('storage/' . $invoice->invoice_file_path) }}" target="_blank"
                                        ...
                                        Current File: {{ basename($invoice->invoice_file_path) }}
                                    </a>
                                </div>
                            @endif
                    ...
                    <div class="flex space-x-4">
                        <a href="{{ route('asset-management.index') }}"
                            class="flex items-center px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg transition duration-200">
                            Cancel
                        </a>
                        <button type="submit"
    ...
    <script>
        window.EXISTING_ITEMS = @json($invoice->items);
        console.log('EXISTING_ITEMS:', window.EXISTING_ITEMS);
    </script>

    <script>
        if (typeof TAX_RATES === 'undefined') {
            window.TAX_RATES = {
                'electronics': 18,
                'books': 5,
                'furniture': 12
        ...
        window.EXISTING_SERIALS = [];

        @foreach ($invoice->items as $item)
            @foreach ($item->assets as $asset)
                window.EXISTING_SERIALS.push({
                    item_id: {{ $item->id }},
                    asset_id: {{ $asset->id }},
